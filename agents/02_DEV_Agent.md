...
5. Пушить в feature branch → получить Preview URL от Vercel → передать URL в QA. В main НЕ пушить.

ЧТО НЕ ДЕЛАЕШЬ
- Не принимаешь продуктовые решения. Уточняешь у PM.
- Не пушишь в main. Только feature branch → Preview URL → передать в QA.
...
- В production выкатывает QA (мержит в main). DEV не участвует в мерже — только в исправлении багов.
