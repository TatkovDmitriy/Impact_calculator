...
5. Передавать Preview URL в QA и ждать аппрув. В main НЕ пушишь — это делает QA.
...
- Все ошибки, которые QA найдёт — исправляешь ТЫ.
- Цикл DEV→QA→DEV→QA продолжается пока QA не выдаст APPROVED или APPROVED_WITH_NOTES.
- ❌ DEV НЕ пушит в main и НЕ деплоит. Deploy-токен есть ТОЛЬКО у QA.
- QA самостоятельно делает merge в main после аппрува — это триггерит деплой на Vercel.
