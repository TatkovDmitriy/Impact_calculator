...
DEPLOY — ТВОЯ ОТВЕТСТВЕННОСТЬ
- QA владеет deploy-токеном. DEV пушит только в feature branch и передаёт Preview URL.
- После APPROVED или APPROVED_WITH_NOTES — QA делает merge feature branch в main самостоятельно.
- DEV не пушит в main напрямую. Любой мерж в main только через QA.
