...
// Hardcoded April 2026 actuals from docs/05_Metrics_Source.md
// Real parser (lib/sheets/parser.ts) will replace this in Phase 3
const APRIL_2026_BASELINE: NovoselBaseline = {
  byCategory: {
    bathroom: {
      totalCreated: 27429,
      novoselCreated: 5331,
...
const CACHE_DOC = (date: string) => `metrics_cache/${date}`;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!forceRefresh) {
      const cacheDoc = await adminDb.doc(CACHE_DOC(today)).get();
      if (cacheDoc.exists) {
        const cached = cacheDoc.data();
        return NextResponse.json({
          date: today,
          source: 'cache',
          baseline: cached?.baseline ?? APRIL_2026_BASELINE,
        });
      }
    }

    // For Phase 2: return hardcoded April 2026 data
    // Phase 3 will replace this with real Sheets parsing
    const baseline = APRIL_2026_BASELINE;

    // Cache in Firestore
    await adminDb.doc(CACHE_DOC(today)).set({
      source: 'hardcoded',
      baseline,
      fetchedAt: new Date().toISOString(),
    });

    return NextResponse.json({ date: today, source: 'fresh', baseline });
  } catch (error) {
    console.error('[baseline] GET error:', error);
    // Fallback: return hardcoded data even if Firestore fails
    return NextResponse.json({
      date: today,
      source: 'fallback',
      baseline: APRIL_2026_BASELINE,
    });
  }
}
