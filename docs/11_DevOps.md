...
│ (Дмитрий дома)      │   │ HQ3381 (дома+WiFi)  │   │                     │
│ Node 20+, npm       │   │ Python 3.14 global  │   │ Vercel (Next.js)    │
...
│ AmneziaWG (split)   │   │ ⚠️ NO Claude Code   │   │                     │
...
| Локальный путь (рабочий ПК) | `C:\Users\60110579\Impact_calculator\` |
...
### Статус рабочего ПК (HQ3381) — 2026-05-10 ✅ BOOTSTRAP COMPLETE

| Компонент | Статус | Детали |
|---|---|---|
| Удалённый доступ | ✅ AnyDesk | RDP заблокирован GPO (пользователь не в Remote Desktop Users) |
| Git | ✅ 2.53.0 | |
| Python | ✅ 3.14.4 (global) | `C:\Users\60110579\AppData\Local\Programs\Python\Python314\` |
| pip-пакеты | ✅ установлены | psycopg2-binary, firebase-admin, pandas, python-dotenv |
| Репозиторий | ✅ склонирован | `C:\Users\60110579\Impact_calculator\` |
| Firebase API | ✅ Path A | firestore.googleapis.com:443 открыт |
| research/.env | ✅ создан | GP creds + Firebase Admin key, UTF-8 no BOM |
| pre-commit hook | ✅ активирован | `git config core.hooksPath ops/hooks` |
| health checks | ✅ все зелёные | VPN OK / Greenplum OK / Firebase OK, exit 0 |
...
| P-05 | RDP к рабочему ПК — «учётная запись не имеет прав для удалённого входа» | Корпоративный GPO не добавляет пользователей домена в Remote Desktop Users | Использовать **AnyDesk** (устанавливается без прав администратора, не зависит от Windows RDP) |
| P-06 | ping/RDP к рабочему ПК недоступен с личного ПК при активном VPN | AmneziaWG с `AllowedIPs = 0.0.0.0/0` перехватывает локальный трафик 192.168.x.x | Настроить split-tunnel: заменить AllowedIPs на CIDR-список исключающий 10/8, 172.16/12, 192.168/16 |
