...
Запускать на корп ПК: cd research && python scripts/segment_novosel_repeat_buyers/publish.py
"""
...
def main() -> None:
    ...
    segments = df['client_segment'].unique().tolist()
    assert 'Новосел' in segments, f'Сегмент Новосел не найден: {segments}'
    months = df['deal_month'].nunique()
    log.info('[sanity] Строк: %d, сегментов: %s, месяцев: %d', len(df), segments, months)

    log.info('Вычисляю payload...')
    payload = compute_payload(df)

    kpi_map = {i['label']: i['value'] for i in payload['blocks'][0]['items']}
    assert kpi_map['Уникальных клиентов-Новоселов'] > 0, 'Клиентов-Новоселов = 0'
    log.info('[sanity] Новоселов: %d, avg_paid: %.2f',
             kpi_map['Уникальных клиентов-Новоселов'],
             kpi_map['Сред. оплаченных проектов (Новосел)'])

    doc = {
        'slug':        SLUG,
        'title':       'Новоселы: частота покупок vs База (дек 2025 – апр 2026)',
